...
void updatePlayer() {
    ...
    if (player.screenCol + player.width >= SCREENWIDTH && player.momentum > 0) {
        player.momentum = 0;
    } else if (player.screenCol <= 0 && player.momentum < 0) {
        player.momentum = 0;
    }

    player.worldCol += player.momentum;
    // player.phOff += player.momentum/2;
